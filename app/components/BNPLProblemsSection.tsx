...
export default function BNPLProblemsSection() {
  return (
    <section className="w-full py-20 px-2 sm:px-6 lg:px-16 flex flex-col items-center relative overflow-hidden scroll-reveal" style={{ background: "linear-gradient(135deg, #101010 60%, #232323 100%)" }}>
      ...
      <div className="absolute inset-0 pointer-events-none z-0" style={{ background: "radial-gradient(ellipse at 30% 10%, rgba(34,197,94,0.10) 0%, transparent 70%), radial-gradient(ellipse at 80% 90%, rgba(34,197,94,0.08) 0%, transparent 70%)" }} />
      <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center">
        <h2 className="text-center text-2xl sm:text-4xl lg:text-5xl font-bold text-[#22c55e] mb-4 sm:mb-6 leading-tight drop-shadow-lg transition-smooth">
          Why Traditional BNPL <span className="text-gradient">Falls Short</span>
        </h2>
        <p className="text-center text-gray-400 text-base sm:text-lg max-w-2xl mb-8 sm:mb-12 transition-smooth">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 w-full transition-smooth">
          {problems.map((problem) => (
            <ProblemCard
              key={problem.title}
              title={problem.title}
              description={problem.description}
              className="shadow-xl bg-[#181818]/80 border border-[#22c55e]/10 hover:border-[#22c55e]/30 hover:scale-[1.03] transition-all duration-300 scroll-reveal min-h-[130px]"
            />
          ))}
        </div>
      </div>
